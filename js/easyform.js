 ...
 * 6. 2016-11-10 修改验证方式，如果非必填，不填值不验证
 * 7. 2016-11-11 增加ajaxForm,以及更多自定义验证方式
 *
 * */
;
                ...
                if (input.type != "hidden" && input.type != "button" && input.type != "submit")
                {
                    ...
                    checker.success = function (e)
                    {
                        ...
                        if (ev.counter == ev.inputs.length)
                        {
                            ...
                            if (!!ev.is_submit)
                            {
                                var easyajax = ev.form.attr("easyajax");
                                var type =  ev.form.attr("method") || "get";
                                if(!!easyajax){//ajaxform
                                    ev._parse(easyajax);
                                    var settings = {
                                        "type" : type,
                                        "url" : ev.form.attr("action"),
                                        "data" : ev.form.serialize(),
                                        "async" : ev.ajaxs.async != "false"
                                    };
                                    if(!!ev.ajaxs.type){
                                        settings.dataType = ev.ajaxs.type;
                                    }
                                    settings.success = function(data){
                                        if(!!ev.ajaxs.target){
                                            $("#"+ev.ajaxs.target).html(data);
                                        }else{
                                            eval(ev.ajaxs.callback+"(data)");
                                        }
                                    }
...
(function ($, window, document, undefined)
{
    ...
    var _easyinput = function (input, opt)
    {
        ...
        this.real_time = true;//默认失去焦点执行验证
    };
    ...
    _easyinput.prototype = {
        ...
        init: function ()
        {
            ...
            if (this.real_time || !!rule && -1 != rule.indexOf("real-time"))
            {
        ...
        validation: function ()
        {
            ...
            if(this.rule.indexOf("require") != -1 || this.rule.indexOf("req") != -1){//是否必填
                if(this.input[0].type=="checkbox" || this.input[0].type=="radio" ){//多选框、单选框，是否必选
